...
The backbone of this course is an 8-step modeling framework adapted from engineering and the physical sciences. In those fields, the framework guides the construction of models for heat transfer, fluid dynamics, chemical reactions, and similar physical processes. The logic, however, is entirely general. Any phenomenon that can be described formally---including behavioral phenomena---can be modeled using this structure.

The framework is not a rigid recipe to be followed once and discarded. It is a disciplined habit of thought. Each step forces the modeler to make a decision, and each decision is recorded so that others can evaluate it. The result is a model that is transparent, testable, and improvable.

Below, each step is presented first in its original engineering language and then translated into behavior-science terms, followed by a definition, explanation of why it matters, a behavioral example, and common mistakes.

## The Translation Table

| Step | Engineering / Physics Language | Behavior-Science Language |
|------|-------------------------------|--------------------------|
| 1 | Get the physical picture clearly in mind | Get the behavioral phenomenon clearly in mind |
| 2 | Define the physical processes and boundaries | Define the behavioral processes and scope of the model |
| 3 | Write down the laws and transport functions | Identify behavioral principles, quantitative laws, and functional relationships |
...
**Why it matters.** A model of delay discounting requires understanding what happens when organisms choose between smaller-sooner and larger-later reinforcers. A model of demand requires understanding how consumption changes with price. Skipping this step leads to models that are mathematically elegant but behaviorally meaningless.

**Behavioral example.** You want to model how pigeons allocate pecks across two concurrently available VI schedules. Before writing any equations, you must be able to describe the procedure (two keys, two independent VI schedules, a changeover delay), the typical data pattern (relative response rates roughly track relative reinforcement rates), and the conditions under which the pattern holds.
...
**Definition.** No model captures everything. The modeler must decide which behavioral processes are in scope and which are excluded. Will the model address only steady-state responding, or also acquisition and extinction? Does it cover a single operant class or multiple concurrent operants?

**Why it matters.** Defining boundaries is not a weakness---it is a disciplined decision about what the model is intended to explain. A model that tries to explain everything typically explains nothing well. Clear scope also tells readers what the model is *not* claiming.
...
**Common mistakes.** Making scope too broad (trying to explain all of choice behavior with one equation). Making scope too narrow without acknowledging what is excluded. Failing to state the scope at all, leaving readers to guess what the model is supposed to cover.

### Step 3: Identify Behavioral Principles, Quantitative Laws, and Functional Relationships

**Definition.** In physics, one might invoke Newton's laws or Fourier's law of heat conduction. In behavior science, the analogues are established quantitative relationships: the matching law, the demand function, the discounting function, principles of reinforcement and punishment, Rescorla-Wagner learning rules, and so forth.
...
**Behavioral example.** Common assumptions in behavior-science models include: steady-state responding, independence of alternatives, constant motivation, absence of extraneous reinforcement, perfect detection of responses, and stable preference.
...
**Definition.** In engineering, a "balance" refers to an accounting of inputs, outputs, and accumulations (e.g., mass balance, energy balance). In behavior science, the analogue is a formal accounting of the behavioral processes specified in earlier steps. First describe in plain language what the model claims, then write the equation.
...
**Why it matters.** Unit checking is a simple but powerful error-detection tool. It also deepens understanding: if you cannot state the units of every term in your model, you may not fully understand what the model claims. Many published errors in behavioral modeling could have been caught by a unit check.
...
**Why it matters.** A model that has not been tested is a conjecture. Verification catches mathematical errors. Validation catches theoretical errors. Solving is what makes the model useful---it is how you generate the predictions that motivate experiments and applications.
...
A food-deprived rat presses a lever in an operant chamber. Presses are reinforced with food pellets on a variable-interval schedule. Across conditions, the VI parameter changes (e.g., VI 30-s, VI 60-s, VI 120-s, VI 240-s), producing different rates of reinforcement. After extended exposure to each condition, the rat's response rate stabilizes. The empirical regularity: response rate increases with reinforcement rate, but the relationship is negatively accelerated---doubling the reinforcement rate does not double responding.
...
4. Motivation (deprivation level) is constant across conditions.
...
In plain language: the equation describes a hyperbolic curve. When reinforcement is low relative to extraneous reinforcement, responding is low. As reinforcement increases, responding rises but gradually levels off at a ceiling of $k$. The parameter $r_e$ governs the curvature---larger $r_e$ means more extraneous reinforcement competing with the measured source, so it takes more reinforcement to drive responding toward the ceiling.
...
**Validate.** Fit the model to data from de Villiers and Herrnstein (1976), who compiled VI schedule data from multiple species and laboratories. Assess fit using $R^2$ and residual plots. The model typically accounts for 90% or more of the variance in steady-state VI data.
...
The 8-step framework is presented as a numbered list, but modeling is rarely a strictly linear process. In practice, you will cycle through the steps repeatedly:
...
The framework is a *scaffold*, not a straitjacket. Its value is in ensuring that every important decision gets made and recorded, not in dictating the order of discovery. When you return to an earlier step, update all subsequent steps to maintain consistency. The final product should read as if the steps were followed in order, even if the actual process involved considerable back-and-forth.

By the end of this course, the framework should be internalized as a habit of thought---not merely a checklist to consult, but a way of thinking about how to move from a behavioral phenomenon to a formal, testable model.
