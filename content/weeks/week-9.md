...
$$
where:
- $\tau^2$ is the between-group (Level 2) variance (i.e., the variance of group means around the grand mean)
- $\sigma^2$ is the within-group (Level 1) variance (i.e., the variance of individual observations around their group mean)
...
![The same data fit with fixed effects, a random intercept, and a random slope](/images/week9-random-effects.svg)
...
![Complete pooling, no pooling, and partial pooling of participant means](/images/week9-shrinkage.svg)
...
Substituting the Level 2 equation into Level 1 gives the combined equation above. The beauty of this framework is that it separates within-person variability ($\sigma^2$) from between-person variability ($\tau^2$), and the ICC can be directly computed from these two variance components.
...
- $\phi$ is the autoregressive coefficient, capturing the magnitude of temporal dependence
- $\epsilon_t$ is white noise (independent, identically distributed error), with $\epsilon_t \sim N(0, \sigma^2_\epsilon)$

The AR(1) model says that the current value equals a fraction of the previous value, plus random noise. If $\phi = 0.8$, then 80% of the current value is "inherited" from the previous time point, and 20% is new random input.
...
![AR(1) series across values of the autoregressive coefficient](/images/week9-ar1-phi.svg)

*Figure: Five AR(1) series generated from the same white noise, varying only $\phi$.*
...
![ACF and PACF signatures of AR(1) and MA(1) processes](/images/week9-acf-pacf.svg)
...
![Additive decomposition of a 90-day self-monitoring record](/images/week9-decomposition.svg)

*Figure: The 90-day record described above, decomposed additively into $x_t = T_t + S_t + R_t$.*
...
- **AR(1) is the simplest time-series model.** $x_t = \phi \cdot x_{t-1} + \epsilon_t$. The parameter $\phi$ captures the magnitude of temporal persistence.
